...
public class AiService {
    ...
    public List<String> expandQuery(String userQuery) {
        ...
        String prompt = String.format(
                "Ты — лингвистический анализатор. Извлеки из запроса ВСЕ знаменательные слова (включая аббревиатуры), " +
                        "игнорируя предлоги, союзы, частицы, междометия, местоимения, вводные слова, вопросительные слова, модальные слова, слова-паразиты и одиночные буквы с точками. " +
                        "Аббревиатуры (например, КНР, США) расшифруй полностью, для каждого слова расшифровки выдай словоформы. " +
                        "Если видишь аббревиатуру (например, КНР, США, СВО), ОБЯЗАТЕЛЬНО расшифруй её полностью, а затем выдай словоформы для каждого слова расшифровки. " +
                        "Исходную аббревиатуру НЕ включать в ответ. " +
                        "Для КАЖДОГО слова выдай не более 10 основных словоформ (падежи, число, род) и усечённую основу (корень без окончания). " +
                        "Пиши ТОЛЬКО словоформы через запятую, без лишних слов, без пересказа инструкции.\n" +
                        ...
                        "Пример для запроса \"Д.Трамп\": Трамп, Трампа, Трампу, Трампом, Трампе\n" +
                        "Запрос: %s\nОтвет:",
                safeQuery
        );
